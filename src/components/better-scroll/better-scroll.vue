        ...
        >
          <div class="pullup-wrapper" v-if="pullUpLoad">
            <div class="before-trigger" v-if="!isPullUpLoad">
              <span class="load-txt">{{ pullUpTxt }}</span>
            </div>
    ...
    >
      ...
      >
        ...
        <div class="after-trigger" v-else>
          ...
          <div v-else>
            <span class="load-txt">{{ refreshTxt }}</span>
          </div>
        </div>
      </div>
    </slot>
  ...
  methods: {
    ...
    forceUpdate(dirty) {
      if (this.pullDownRefresh && this.isPullingDown) {
        this.pullUpDirty = true; // 开启上拉加载事件
        this.scroll.openPullUp(); // 刷新之后重新开启上拉加载
        this.isPullingDown = false; // 是否刷新
        this.scroll.closePullDown(); // 关闭下拉刷新,修复异步回调BUG。
    ...
    _afterPullDown() {
      setTimeout(() => {
        ...
        this.scroll.openPullDown(); //重新开启刷新
        this.refresh();
