    ...
    config.plugins = config.plugins.concat([{
      apply: (compiler) => {
        if (scriptActive === "watch-build") {
          /** 插件初始化后开启wifi服务 */
          compiler.hooks.afterPlugins.tap('apicloud', compilation => {
          ...
          compiler.hooks.afterEmit.tap('apicloud', compilation => {
            ...
            const unlinked = [];
            const files = fs.readdirSync(path.join(__dirname,`./${appname}/`));
            if (files.length) {
          ...
          /** 编译完成，真机同步 */
          compiler.hooks.done.tap('apicloud', compilation => {
            exec(
              ...
              (error, stdout) => {
                ...
                console.log("\033[35m"+'【APICloud-cli】:wifi真机同步成功'+ stdout.toString()+"\033[0m\n")
              }
            );
          });
        }
